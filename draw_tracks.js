...
function drawTracks() {
    ...
    d3.csv("/src/a_01fld07_05-09-2021-12-48-25.csv").then(trackData => {
        ...
        trackDataToCurr.filter(d => d.FRAME < imgIndexScaled)
            .forEach(d => {
                trackPaths[d.track_id_unique - 1].push([scaleX(d.pos_x), scaleY(d.pos_y)]);
                trackPaths[d.track_id_unique - 1].push([scaleX(+d.pos_x + +d.dt1_n0_dx), scaleY(+d.pos_y + +d.dt1_n0_dy)]);
                trackPaths[d.track_id_unique - 1].push([scaleX(+d.pos_x + +d.dt2_n0_dx), scaleY(+d.pos_y + +d.dt2_n0_dy)]);
            });
        trackDataToCurr.filter(d => d.FRAME == imgIndexScaled)
            .forEach(d => {
                trackPaths[d.track_id_unique - 1].push([scaleX(d.pos_x), scaleY(d.pos_y)]);
        ...
        startPointsGroup.selectAll("circle")
            ...
            .enter()
            .append("circle")
            .attr("class", d => `${trackPaths.indexOf(d) + 1}`)
