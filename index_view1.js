...
const createView1SVG = () => {
    for (let i = 0; i < numDataset; i++) {
        ...
        div.append("h1").text(`dataset #${i + 1}`)
        view1ErrTrkGroupArr[i] = div.append("svg")
...
const drawErrorTrack = (datasetIdx) => {
    ...
    if (pathData.length === 0) {
        const text = view1ErrTrkGroupArr[datasetIdx].append("text")
            .attr("id", `noErrorText${datasetIdx}`)
            .attr("y", resolutionSideLength / 2)
            .attr("style", "font: 50px sans-serif")
            .text("Congratulation, this dataset has no error!");
        const tempWidth = document.getElementById(`noErrorText${datasetIdx}`).getBBox().width;
