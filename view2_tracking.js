...
function highlight() {
    ...
    if (seletedHTMLCollection != htmlCollection) {
        ...
        treeGroup.select(`#TrackID${this.getAttribute("class").split("-")[0]}`).attr("stroke", ERR_TRK_COLOR);
    }
}
function unhighlight() {
    ...
    if (seletedHTMLCollection != htmlCollection) {
        ...
        treeGroup.select(`#TrackID${this.getAttribute("class").split("-")[0]}`).attr("stroke", treeColor);
    }
}
...
function drawErrorTrack() {
    ...
    for (let i = 0; i < pathData.length; i++) {
        ...
        const circles = group.selectAll("circle")
            ...
            .attr("fill", (d, ii) => seletedHTMLCollection && !isErrorSelected(idxToErrTrkIDArr[i], ii) ? "none" : ERR_TRK_COLOR);
        circles.exit()
            .attr("r", undefined);
        circles.enter()
            .append("circle")
        ...
        const paths = group.selectAll("path")
            .data(pathData[i])
            .attr("d", d => d3.line()(d))
            .attr("stroke", (d, ii) => seletedHTMLCollection && !isErrorSelected(idxToErrTrkIDArr[i], ii) ? undefined : ERR_TRK_COLOR);
        paths.exit()
            .attr("d", undefined);
        paths.enter()
            .append("path")
    ...
    if (seletedHTMLCollection) {
        const classInfo = seletedHTMLCollection[0].attributes.class.value.split("-");
