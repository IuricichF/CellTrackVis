...
function unhighlightMouseoutTrack() {
    ...
    if (selectedHTMLCollectionByTrack !== htmlCollection) {
        ...
        if (highlightedHTMLCollectionByTree.length === 0) treeGroup.select(`#TrackID${this.getAttribute("class").split("-")[0]}`).attr("stroke",
            scaleTreeColor(trkIDToErrImgIdxMap.get(+this.getAttribute("class").split("-")[0]).length));
    }
}
